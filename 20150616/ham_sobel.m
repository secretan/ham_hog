...
% OutImage大小统计
% Vetor梯度方向
function [OutImage, Vetor] = ham_sobel(InImage)
[m,n] = size(InImage);
parray = zeros(1,9);
    ...
    for j = 1:n
        if i == 1 
            if j == 1
                ...
                parray(1,4) = InImage(i,j);
                parray(1,7) = InImage(i,j);

                parray(1,8) = InImage(i,j+1);
                parray(1,9) = InImage(i+1,j+1);
            elseif j == n
                parray(1,1) = InImage(i,j);
                parray(1,4) = InImage(i,j);
                parray(1,7) = InImage(i,j);
                parray(1,9) = InImage(i,j);
                parray(1,8) = InImage(i,j);
            ...
            parray(1,5) = InImage(i,j);
            parray(1,6) = InImage(i+1,j);
        elseif i==m
            if j == 1
                parray(1,1) = InImage(i,j);
                parray(1,2) = InImage(i,j);
                parray(1,3) = InImage(i,j);
                parray(1,6) = InImage(i,j);
                parray(1,9) = InImage(i,j);

                parray(1,8) = InImage(i,j+1);
                parray(1,7) = InImage(i-1,j+1);
            elseif j == n
                parray(1,3) = InImage(i,j);
                parray(1,6) = InImage(i,j);
                parray(1,9) = InImage(i,j);
            ...
            if j == 1
                ...
                parray(1,9) = InImage(i+1,j+1);
            elseif j == n
                parray(1,7) = InImage(i-1,j);
                parray(1,8) = InImage(i,j);
                parray(1,9) = InImage(i+1,j);
                parray(1,1) = InImage(i-1,j-1);
                parray(1,2) = InImage(i,j-1);
                parray(1,3) = InImage(i+1,j-1);
            else                    
